...
MIN_PRICE = 0.0001
MAX_PRICE = 10000.0
GRID_SIZE_USDT = 5.0  # Each grid level ~$5
MAX_SCALED_COINS = 5   # Fewer due to US regulations/limited pairs
RSI_OVERSOLD = 30
RSI_OVERBOUGHT = 70
TRAILING_STOP_PCT = 0.015  # 1.5%
TRAILING_BUY_PCT = 0.02    # 2%
MAX_POSITIONS = 10         # Conservative for spot
POSITION_SIZE_USDT = 25.0
FEE_MAKER = 0.0010         # Binance.US spot maker fee (0.10%)
FEE_TAKER = 0.0010         # Binance.US spot taker fee (0.10%)
# No leverage on spot
...
class Position(Base):
    ...
    side = Column(String(10), nullable=False)  # 'long' only for spot
    leverage = Column(Integer, default=1)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    status = Column(String(20), default="open")  # open, closed, trailing
...
def load_config() -> Dict:
    """Load bot state from config file."""
...
def get_binance_client() -> Client:
    ...
    try:
        client = Client(API_KEY, API_SECRET, tld='us')  # Key: tld='us' for Binance.US
        # Test connectivity
...
class SmartTradingBot:
    ...
    def refresh_symbols(self):
        """Fetch and filter valid USDT trading pairs from Binance.US."""
        try:
            ...
            for s in exchange_info['symbols']:
                if (s['quoteAsset'] == 'USDT' and
                    ...
                    s['baseAsset'] not in ['UST', 'BUSD', 'LUNA'] and  # Avoid delisted/stablecoin issues
                    float(s['filters'][2]['minPrice']) >= MIN_PRICE and  # PRICE_FILTER index 2
                    float(s['filters'][3]['minQty']) > 0):  # LOT_SIZE index 3
                    min_qty = float(s['filters'][3]['minQty'])
                    tick_size = float(s['filters'][2]['tickSize'])
                    symbols.append(s['symbol'])
                    self.valid_symbols_dict[s['symbol']] = {
                        'min_qty': min_qty,
                        'tick_size': tick_size,
                        'volume': 0  # Will update later
                    }
            self.valid_symbols = set(symbols)
            logger.info(f"Loaded {len(self.valid_symbols)} USDT pairs from Binance.US")
        except Exception as e:
    ...
    def get_order_book_analysis(self, symbol: str) -> Dict:
        """Analyze order book for depth/imbalance."""
    ...
    def get_macd_bullish_with_histogram(self, symbol: str) -> Tuple[bool, Optional[float], int, Optional[float]]:
        """MACD analysis with histogram for bullish signal."""
        try:
    ...
    def get_trade_fees(self, symbol: str) -> Tuple[float, float]:
        """Get fees (static for Binance.US spot)."""
    ...
    def place_market_buy(self, symbol: str, amount_usdt: float) -> Optional[str]:
        ...
        try:
            ...
            step_size = info.get('min_qty', 0.000001)
            quantity = round(quantity - (quantity % step_size), 8)

            if quantity * price < 10:  # Binance.US min order ~$10
                logger.warning(f"Order too small for {symbol}: {quantity * price:.2f} USDT")
                return None
    ...
    def get_top_priority_coins(self, limit: int = MAX_SCALED_COINS) -> List[str]:
        """Select top coins for grid based on momentum/RSI/volume."""
        priorities = []
        for sym in list(self.valid_symbols)[:50]:  # Sample top 50
            ob = self.get_order_book_analysis(sym)
            if ob['best_bid'] == 0:
                continue
            rsi, trend, _ = self.get_rsi_and_trend(sym)
            momentum_ok, _, lookback, hist = self.get_macd_bullish_with_histogram(sym)
            low, high, pct_change = self.get_24h_price_stats(sym)
            ...
            if momentum_ok and lookback <= 4:
                score += 40
            if ob['weighted_pressure'] < -0.001:  # Buy pressure
                score += 20
            if pct_change > -5:  # Not crashing
                score += 10
    ...
    def place_scaled_grids(self):
        """Place infinity grid orders on top coins."""
        top_coins = self.get_top_priority_coins()
        balance = float(self.get_balance('USDT'))
        levels = min(8, int(balance // 25))  # Conservative

        for sym in top_coins:
            if sym in self.active_grid_symbols:
                continue
            if balance < 50:
                break
            ...
            grid = {
                ...
                'levels': levels,
                'grid_pct': 0.01,  # 1% spacing
                'buy_orders': [],
                'sell_orders': []
            }

            # Place buy grids below current
            for i in range(1, levels + 1):
                buy_price = current_price * (1 - i * grid['grid_pct'])
                qty = GRID_SIZE_USDT / buy_price
                info = self.valid_symbols_dict.get(sym, {})
                step_size = info.get('min_qty', 0.000001)
                qty = round(qty - (qty % step_size), 8)
                order_id = self.place_limit_buy(sym, qty, buy_price)  # Assume similar method
                if order_id:
                    grid['buy_orders'].append({'price': buy_price, 'qty': qty, 'id': order_id})

            # Place sell grids above current (if holding position)
            # Simplified: Assume we buy on dips, sell on pumps
    ...
    def place_limit_buy(self, symbol: str, quantity: float, price: float) -> Optional[str]:
        """Place limit buy (similar to sell)."""
        try:
            info = self.valid_symbols_dict.get(symbol, {})
            tick_size = info.get('tick_size', 0.00000001)
            price = round(price - (price % tick_size), 8)
    ...
    def manage_trailing_stops(self):
        """Manage trailing buys/sells."""
        for sym, state in list(trailing_sell_active.items()):
            current = self.get_order_book_analysis(sym)['best_bid']
            ...
            if current <= trail_stop:
                ...
                with DBManager() as sess:
                    ...
                    if pos:
                        self.place_market_sell(sym, float(pos.quantity))  # Assume market sell
                        pos.status = 'closed'
    ...
    def place_market_sell(self, symbol: str, quantity: float) -> Optional[str]:
        ...
        try:
            ...
            order_id = order['orderId']
            logger.info(f"Market SELL {symbol}: {quantity:.6f}")
...
if __name__ == "__main__":
    ...
    # Option: Run dashboard in thread
    def dashboard_loop():
        while True:

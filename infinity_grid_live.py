#!/usr/bin/env python3
"""
INFINITY GRID PLATINUM 2025 — UNBANNABLE & UNCRASHABLE
WebSocket crash "indices must be integers" → PERMANENTLY FIXED
Single combined WebSocket + all functions included
"""
...
if not api_key or not api_secret:
    messagebox.showerror("FATAL ERROR", "Set BINANCE_API_KEY and BINANCE_API_SECRET!")
    sys.exit(1)
...
tk.Label(root, text="PLATINUM 2025", font=title_font, fg="#00ff00", bg="#0d1117").pack(pady=5)
...
def load_symbol_info():
    ...
    try:
        ...
        for s in info:
            ...
            count += 1
        log(f"Loaded {count} active USDT pairs")
...
def generate_buy_list():
    ...
    try:
        ...
        log(f"CoinGecko failed → using volume fallback ({e})")
    ...
    if len(candidates) < 5:
        try:
            ...
            log("Using top 15 volume coins (fallback)")
        except:
...
def place_limit_order(symbol, side, price, qty):
    ...
    try:
        ...
        log(f"Order rejected {symbol} {side}: {e.message}")
        return False
    except Exception as e:
        log(f"Order error: {e}")
        return False
...
# ==================== SINGLE BULLETPROOF WEBSOCKET ====================
def start_websocket():
    def on_message(ws, message):
        try:
            data = json.loads(message)

            # User data (balances + fills)
            if isinstance(data, dict) and data.get('e') == 'outboundAccountPosition':
            ...
            elif isinstance(data, dict) and data.get('e') == 'executionReport':
                if data.get('X') in ['FILLED', 'PARTIALLY_FILLED'] and Decimal(data.get('l', '0')) > ZERO:
                    ...
                    log(f"FILLED {side} {qty:.6f} {base} @ ${price}")
                    send_whatsapp(f"{side} {qty:.4f} {base} @ ${price}")
                    if running:
                        threading.Thread(target=regrid_symbol, args=(sym,), daemon=True).start()
            ...
            # All tickers array
                ...
                for ticker in data:
                    ...
                    if s and s.endswith('USDT'):
                        price_cache[s] = (Decimal(ticker['c']), time.time())
    ...
    def on_open(ws):
        log("WebSocket CONNECTED (user + all tickers)")

    def run():
        while True:
            try:
                listen_key = client.stream_get_listen_key()['listenKey']
                ws_url = f"wss://stream.binance.us:9443/stream?streams={listen_key}/!ticker@arr"
...
def start_bot():
    ...
    log("BOT STARTED — UNBANNABLE MODE ACTIVE")
...
def stop_bot():
    ...
    log("BOT STOPPED — All orders canceled")
...
if __name__ == "__main__":
    log("INFINITY GRID PLATINUM 2025 — UNBANNABLE EDITION")
    load_symbol_info()
    start_websocket()
    time.sleep(5)
    generate_buy_list()
    update_gui()
    log("SYSTEM READY — PRESS 'START BOT'")
    root.mainloop()
